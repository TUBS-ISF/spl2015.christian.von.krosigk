...
public class MP3Gui extends JFrame implements ActionListener { //, ListSelectionListener {
	...
	//Plugins
	private ShowCover 		coverPlugin = null;
	private ShowCurrentSong songPlugin = null;
	private VolumeControl 	volumePlugin = null;
	private Mute 			mutePlugin = null;
	private ProgressBar		progressPlugin = null;
	private ShowTime 		timePlugin = null;
	...
	public void execute() {
		...
		if (progressPlugin != null) {
			int progWidth = progressPlugin.getWidth();
			int progHeigth = progressPlugin.getHeight();
			progBar.setSize(progWidth,progHeigth);
			// progBar.setStringPainted(true);
		}
		
		// Volumebar
		if (volumePlugin != null) {
			int sliderMin = volumePlugin.getSliderMin(),
				sliderMax = volumePlugin.getSliderMax(),
				initVal   = volumePlugin.getStartValue();
			
			volBar = new JSlider(sliderMin, sliderMax);
			volBar.setName("Volume:");
	
			volBar.setValue(initVal);
		}
		else {
		...
		volBar.addChangeListener(new ChangeListener() {
		...
		// Cover Label
		if (coverPlugin != null) {
			int CoverWidth = coverPlugin.getWidth();
			int CoverHeight = coverPlugin.getHeight();
			String CoverPath = coverPlugin.getPathDefaultPciture();
			default_coverIcon = new ImageIcon(CoverPath);
			Image img = default_coverIcon.getImage();
			img = img.getScaledInstance(CoverWidth, CoverHeight, Image.SCALE_SMOOTH);
			default_coverIcon = new ImageIcon(img);
			coverLabel = new JLabel(default_coverIcon);
		}
		else {
			coverLabel = new JLabel(DEFAULT_PICTURE_PATH);
		}
	...
	public void addPlugin(Plugin p) {
		String plugName = p.getPluginName();
		
		if (plugName.equals("ShowCover")) {
			coverPlugin = (ShowCover) p;
		}
		else if (plugName.equals("ShowCurrentSong")) {
			songPlugin = (ShowCurrentSong) p;
		}
		else if (plugName.equals("VolumeControl")) {
			volumePlugin = (VolumeControl) p;
		}
		else if (plugName.equals("Mute")) {
			mutePlugin = (Mute) p;
		}
		else if (plugName.equals("ProgressBar")) {
			progressPlugin = (ProgressBar) p;
		}
		else if (plugName.equals("ShowTime")) {
			timePlugin = (ShowTime) p;
		}
